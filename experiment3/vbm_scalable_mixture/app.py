...
flags.DEFINE_enum(
    "mode", 'train', ['train', 'eval', 'generate', 'generate_by_cluster', 'reconstruct', 'iterative_read', 'copy'],
    "mode: one of train, eval, generate, generate_by_cluster, reconstruct, iterative_read, copy")
...
flags.DEFINE_integer("opt_iters", 10, "opt_iters: number of iterations to run writing algorithm; also used during iterative reading to optimize q(s)q(w|s)")
...
flags.DEFINE_float("sr_delta", 2e-6, "sr_delta: delta hyperparameter for the stochastic regularization method introduced in section 4.3")
flags.DEFINE_float("sr_epsilon", 1e-6, "sr_epsilon: epsilon hyperparameter for the stochastic regularization method introduced in section 4.3")
...
flags.DEFINE_string("summaries_dir", '/tmp/vbm_gmm_kmpp_swishgroupconvres_nsr_summaries/', "summaries_dir: directory for tensorboard logs")
...
def main(_):
    ...
    mode_to_routine = {
        ...
        'generate_by_cluster': routines.generate_by_cluster,
