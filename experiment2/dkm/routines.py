...
def generate(dataset, sess, model, callbacks):
    ...
    for batch in tfds.as_numpy(dataset):
        ...
        memory_state = model.get_memory_state(sess, batch)

        mem_fp = callbacks['save_png'](np.expand_dims(np.concatenate([memory_state.R[0], memory_state.U[0]], axis=1), -1))
        print(mem_fp)
...
def reconstruct(dataset, sess, model, callbacks):
    for batch in tfds.as_numpy(dataset):
        ...
        for i in range(0, episode_len):
            x = batch[:, i]  # [1, h, w, c]
            img = model.read_from_memory_state(sess, memory_state, x)
            img = np.squeeze(img, 0)
            read_xs.append(img)
    ...
    for batch in tfds.as_numpy(dataset):
        ...
        for _ in range(0, num_progressions):
            ...
            for iter_ in range(0, progression_len * viz_dilation_rate):
                x = query_xs[-1]
                x = model.read_from_memory_state(sess, memory_state, x)
                query_xs.append(x)
