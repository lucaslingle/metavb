...
import variational_bayesian_memory as VBMG
import data as data
...
flags.DEFINE_integer("opt_iters", 50, "opt_iters: number of iterations to run writing algorithm")
...
flags.DEFINE_string("summaries_dir", '/tmp/vbmg_summaries/', "summaries_dir: directory for tensorboard logs")
...
def main(_):
    ...
    ## model and session
    model = VBMG.VariationalBayesianMemory(hps)
    sess = tf.Session()
